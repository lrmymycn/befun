...
public class MyBeanUtils {
        ...
        if (firstParen > -1 && lastParen == -1) {
            ...
            for (int i = 0; i < parameterTypeNames.length; i++) {
                ...
                try {
                    ...
                    throw new IllegalArgumentException("Invalid method signature: unable to resolve type [" + parameterTypeName + "] for argument " + i
                                                       + ". Root cause: " + ex);
                }
            }
    ...
    public static boolean isSimpleValueType(Class<?> clazz) {
        return ClassUtils.isPrimitiveOrWrapper(clazz) || clazz.isEnum() || CharSequence.class.isAssignableFrom(clazz) || Number.class.isAssignableFrom(clazz)
               || Date.class.isAssignableFrom(clazz) || clazz.equals(URI.class) || clazz.equals(URL.class) || clazz.equals(Locale.class)
     ...
     * @see BeanWrapper
     */
    public static void copyPropertiesInclude(Object source, Object target, String[] includedProperties) throws BeansException {
        copyPropertiesInclude(source, target, null, includedProperties);
    }
    ...
    private static void copyPropertiesInclude(Object source, Object target, Class<?> editable, String[] includedProperties) throws BeansException {
        ...
        PropertyDescriptor[] targetPds = getPropertyDescriptors(actualEditable);
        List<String> includedList = (includedProperties != null) ? Arrays.asList(includedProperties) : null;

        for (PropertyDescriptor targetPd : targetPds) {
            if (targetPd.getWriteMethod() != null && (includedProperties == null || includedList.contains(targetPd.getName()))) {
                PropertyDescriptor sourcePd = getPropertyDescriptor(source.getClass(), targetPd.getName());
                if (sourcePd != null && sourcePd.getReadMethod() != null) {
                    try {
                        Method readMethod = sourcePd.getReadMethod();
