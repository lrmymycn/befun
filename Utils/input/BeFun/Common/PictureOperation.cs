        ...
        using (var g = Graphics.FromImage(imgResult))
        {
            ...
            if (model.IsWartermarked)//是否加水印
            {
                ...
                bool doImageWartermark = (null != warterMark
                    && warterMark.Width < destWidth / 3
                    && warterMark.Height < destHeight / 3);//能否图片水印

                if (doImageWartermark)
                {
                    #region 图片水印
                    GetWatermarkPlacement(WatermarkPositions.BottomLeft, g.DpiX, g.DpiY, destWidth, destHeight, warterMark, out x, out y);
...
public enum WatermarkPositions { TopLeft, TopRight, BottomLeft, BottomRight }
